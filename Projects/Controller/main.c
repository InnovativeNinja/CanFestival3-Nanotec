...
int main(int argc, char *argv[])
  {
   ...
   if (argc > 2)
      ...
      dll_file_name = "can_ixxat_win32.dll";
   ...
   if (canOpen(&MasterBoard,&win32test_Data))
      ...
      unsigned char error = masterSendNMTstateChange (&win32test_Data, 0, NMT_Start_Node);

      printf("\nStarting node %d (%xh) ...\n",(int)node_id,(int)node_id);
      
      /* wait untill mode will switch to operational state*/
      if (GetChangeStateResults(node_id, Operational, 5000) != 0xFF)
         {
      ...
      masterSendNMTstateChange (&win32test_Data, 0x2, NMT_Stop_Node);
